...
export class LimiteComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // === PATRIMONIO: inicialización corregida (coincide con lo que parchea el backend) ===
    this.updateTableDataSourcePatrimonio();

    this.parametriaPatrimonioFormGroup = this.formBuilder.group({
      idPatrimonio: [''],
      patrimonio: ['', [Validators.required]],          // valor del patrimonio
      fec_periodo: ['', [Validators.required]],         // fecha del patrimonio
...
/**
            ...
            this.parametriaPatriminioService
              ...
              .subscribe({
                next: () => {
                  ...
                  // Refresh the PATRIMONIO table (corrección: antes refrescaba limites)
                  this.updateTableDataSourcePatrimonio();
                },
                error: (error) => {
                  swal(
  ...
  setParametriaPatrimonioFormGroupById(id: number) {
    ...
    // Mantuve el nombre que ya tenías (setNewParametriaMoneda)
  ...
  /**
   * Get habilitar boton guardar para patrimonio
   */
  ...
  /**
   * This method is called when the save button of patrimonio is clicked
   */
  ...
  createRowParametriaPatrimonio(param: ParametriaPatrimonio) {
    this.isFormLoading = true;
    const toCreate = { ...param } as any;
    delete toCreate.idPatrimonio;
    // asegurar estado = 1
    toCreate.estado = 1;
    ...
    this.parametriaPatriminioService.updateParametriaPatrimonio(param).subscribe({
      next: () => {
        ...
        swal(
          ...
          'El patrimonio ha sido actualizado correctamente.',
          'success'
        );
      },
      error: (error) => {
  ...
  deleteRowParametriaPatrimonio(id: number) {
    ...
    this.parametriaPatriminioService.deleteParametriaPatrimonio(id).subscribe({
      ...
      error: (error) => {
        ...
        this.isTableLoading = false;
        this.isTableFailed = true;
        this.tableStateLabel = "Error al eliminar el patrimonio.";
      },
      complete: () => {
        this.isTableLoading = false;
        this.tableStateLabel = 'Listo.';
